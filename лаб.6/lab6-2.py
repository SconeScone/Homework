""" Цикл while похож на оператор if в том смысле, что он выполняет некоторый код, если выполняется некоторое условие.
 Основное отличие состоит в том, что он будет продолжать выполнять тело цикла до тех пор,
...
""" 
Ключевое слово continue используется для пропуска остальной 
части кода внутри текущего выполняемого цикла и возврата к оператору for или while. 
"""
