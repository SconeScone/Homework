"""
СПИСОК - это структура данных, которую вы можете использовать
 для хранения коллекции различных данных в одной переменной.

Список может быть записан в виде последовательности значений (элементов), разделенных запятыми,
в квадратных скобках, например, lst = [item1, item2]. Списки могут содержать элементы разных типов,
но обычно все элементы в списке одного типа.
...
В отличие от строк, списки являются изменяемым типом, то есть их содержимое можно изменить 
с помощью lst[index] = new_item 
"""
